...
#define FN(return_type, function_body)  \
({                                          \
...
{
    dirTraverse(dirPath, FN(bool, (const char *fileName) {
		if (isExtentionEqual(fileName, "desktop"))
            printAppNameAndIndex(fileName, output, indexMap);
        return true;
    }));
}

void traverseStringArray(const char *s[], void (*action)(const char *))
{
	while (*s)
		action(*(s++));
}
...
void processDmenuSelection(int inputPipe[2], int outputPipe[2], map execMap)
{
	...
	traverseStringArray(desktopAppsPath, FN(void, (const char *dirName) {
		processDirectory(dirName, dmenuFp, &execMap);
	}));
